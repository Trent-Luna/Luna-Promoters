...
const LABEL: Record<string, string> = { promoter: 'Promoter', dj: 'DJ', staff: 'Staff', close_friend: 'Close Friend' }

export function CategorySelect({ id, value }: { id: string; value: 'promoter' | 'dj' | 'staff' | 'close_friend' }) {
  ...
  return (
    <select value={cat} disabled={pending}
      ...
      {(['promoter', 'dj', 'staff', 'close_friend'] as const).map(c => <option key={c} value={c}>{LABEL[c]}</option>)}
    </select>
  )
}
